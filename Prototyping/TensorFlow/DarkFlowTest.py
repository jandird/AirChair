...
if __name__ == '__main__':
    ...
    # INPUT IMAGE HERE
    imgcv = cv2.imread("./resources/test-img/IMG_1287.jpg")
    ...
    for result in results:
        if (result["confidence"] > 0.24):
            ...
            if (result["label"] == 'chair'):
                # rectCentre = (int(round(result["topleft"]["x"]+result["bottomright"]["x"])/2), int(round((result["topleft"]["y"]+result["bottomright"]["y"])/2)))
                # print(rectCentre)
                # cv2.circle(imgcv, rectCentre, 30,(0, 0, 255), -1)
